...
void search_table(int pos){
	int location=0;
	unsigned char k;
	unsigned char lengths[]={0x20,0x22,0x20,0x20,0x20,0x10,0x20,0x22,0x20};
...
int main(void)
{
    ...
    while(1)
    {
		...
		if(count>0)
		{
			...
			printf("Small - Ferromagnetic;       \r");
			}else
			if(detected<0&&detected<-100){
			printf("Large - Ferromagnetic;       \r");
			}else
			if(detected>20&&detected<100){
			printf("Small - Paramagnetic;       \r");
			}else
			if(detected>20&&detected>100){
			printf("Large - Paramagnetic;       \r");
			}else
			
			printf("NO METAL DETECTED       \r");
