...
void main(void)
{
    ...
    while(1)
    {
		...
		if(count>0)
		{
			...
			if(detected<-15||detected>15){
			LATBbits.LATB5 = 1;
			}else
			LATBbits.LATB5 = 0;
			
			if(detected<-50&&detected>-100){
			printf("Small - Ferromagnetic - %f;       \r",detected);
			}else{
			if(detected<-15&&detected>-50){
			printf("Micro - Ferromagnetic - %f;       \r",detected);
			}else{
			if(detected<-100&&detected<-275){
			printf("Medium - Ferromagnetic - %f;       \r",detected);
			}else{
			if(detected<-275){
			printf("Large - Ferromagnetic - %f;       \r",detected);
			}else{
			if(detected>15&&detected<50){
			printf("Micro - Paramagnetic - %f;       \r",detected);
			}else{
			if(detected>50&&detected<100){
			printf("Small - Paramagnetic - %f;       \r",detected);
			}else{
			if(detected>100&&detected<275){
			printf("Medium - Paramagnetic - %f;       \r",detected);
			}else{
			if(detected>275){
			printf("Large - Paramagnetic - %f;       \r",detected);
			}else{
