...
//left out ms timer from detector.c file
...
//numb=number of bytes to playback
...
// Lookup Table function(subroutine)
// Given a position in the Lookup Table call Start_Playback with the apppropriate 
// starting address and nummber of bytes arguments

void search_table(int pos){
	int location=0;
	unsigned char k;
	unsigned char lengths[]={0x20,0x22,0x20,0x20,0x20,0x10,0x20,0x42};
	
	for(k=0; k<pos;k++){
		location+=lengths[k]*0x100;
	}
	Start_Playback(location,lengths[pos]*0x100);
}

void LCDprintDetected(int detected, char* size,int lower, int upper)
{
	unsigned char length=0;
	char buff[17] = "->>>>";
	bool positive = (detected>0)?1:0;
	...
	int increment = abs((upper-lower)/16);
	
	detected-=abs(lower);
	
	while(detected>=increment){
	detected-=increment;
	length++;
	}
...
int main(void)
{
    ...
    CNPDBbits.CNPDB6 = 1;;   // Enable pull-down resistor for RB6
    
    ANSELB &= ~32; // Set RB5 as a digital I/O
    TRISBbits.TRISB5 = 0;   // configure pin RB5 as output
    LATBbits.LATB5 = 0;
    
    ANSELB &= ~32768; // Set RB15 as a digital I/O
    TRISB = 32768;   // configure pin RB15 as input
    CNPUBbits.CNPUB15 = 1;;   // Enable pull-up resistor for RB15
    ...
    while(1)
    {
		...
		if(count>0)
		{
			...
			if(detected<-24&&detected>-50){
			printf("Micro - Ferromagnetic; %f %f %f\r", f, detected, config);
			//detected: varaible representing the detected value
			//size: SIX character string(not including null character) that states the size of this scaled print
			//lower: bound whose absolute value is smaller
			//upper: bound whose absolute value is larger
			
			LCDprintDetected(detected,"Micro ",-24,-50);
			if(silence == 0){
			search_table(1);
			waitms(500);
			search_table(7);
			...
			if(detected<-50&&detected>-500){
			printf("Small - Ferromagnetic; %f %f %f\r", f, detected, config);
			LCDprintDetected(detected,"Small ",-50,-500);
			if(silence == 0){
			search_table(2);
			waitms(500);
			search_table(7);
			...
			if(detected<-500&&detected>-1000){
			printf("Medium - Ferromagnetic; %f %f %f \r", f, detected, config);
			LCDprintDetected(detected,"Medium",-500,-1000);
			if(silence == 0){
			search_table(3);
			waitms(500);
			search_table(7);
			...
			LCDprintDetected(detected,"Large ",-1000,-10000);
			if(silence == 0){
			search_table(4);
			waitms(500);
			search_table(7);
